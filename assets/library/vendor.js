...
var setCarousel = function(){
	function Carousel(element) {
		...
		var list_points = $(">.list-nav", element);
		...
		// if (current_pane == 0) {
		// 	$(">.btn-left", element).hide(0);
		// };

		function detectArrow() {
			if (current_pane == pane_count - 1) {
				$(">.btn-right", element).hide();
			}
			if (current_pane < 1) {
				$(">.btn-left", element).hide();
			}
		}

		function setListPoints() {
			var point = "<span class='lister'></span>";
			for (var i = 0; i <= pane_count - 1; i++) {
		...
		function movePanel(direction) {
			switch (direction) {
				case 0: //right
					if (current_pane <= pane_count - 1) {
						current_pane++;
						var pane_num = current_pane;
						self.showPane(pane_num, true);
						$(">.btn-left", element).fadeIn(400);
					}
				...
				case 1: //left
					if (current_pane > 0) {
						...
						var pane_num = current_pane;
						self.showPane(pane_num, true);
						$(">.btn-right", element).fadeIn(400);
					}
					break;
			}
		}

		$(">.btn-right", element).click(function() {
			movePanel(0);
		});
		$(">.btn-left", element).click(function() {
			movePanel(1);
		...
		function setCurrentPagePointer(index) {
			$('>div.list-nav span', element).removeClass('lister-hover');
			$('>div.list-nav span:eq(' + index + ')', element).addClass('lister-hover');
		}
		setCurrentPagePointer(0); //set the pointerTo 0
		/**
		 * show pane by index
		 */
			...
			switch (ev.type) {
				...
				case 'panend':
					...
					if (current_pane > 0 || current_pane==0) {
						$(">.btn-right", element).show();
					}

					if (current_pane <= pane_count - 1) {
						$(">.btn-left", element).show();
					}
					detectArrow();
		...
		new Hammer(element[0], {
			dragLockToAxis: true
		}).on("pancancel panend panleft panright swipeleft swiperight", handleHammer);
	}
	var carousel_feature = new Carousel("#feature-cases-carousel");
	carousel_feature.init();
